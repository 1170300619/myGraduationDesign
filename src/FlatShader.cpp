#include "../include/FlatShader.hpp"
    ...
    for (const auto &light: *uniform->light_source)
    {
        if (light->type == POINT)
        {
            ...
            if (s > 0)
            {
                ...
                if(lum > MIN_LUM)
                {
                    ...
                    float spec = lum * powf(std::max(frag.flat_normal.dot(bisector), 0.0f), uniform->spec_rank);
        ...
        } else if (light->type == SUN)
        {
            ...
            if (s > 0)
            {
                ...
                float spec = light->luminance * powf(std::max(frag.flat_normal.dot(bisector), 0.0f), uniform->spec_rank);
                float4 specular(spec, spec, spec, 1.0f);
