...
void Camera::set_viewport(int _x, int _y, float ccd_size_x, float ccd_size_y, float _focal)
{
    ...
    ccd_y = ccd_size_y;
    focal = _focal;
    FovH = 2.0f * atanf(ccd_size_x / (2.0f * focal)); //水平视场角
    FovV = 2.0f * atanf(ccd_size_y / (2.0f * focal)); //竖直视场角
...
void Camera::set_viewport(int _x, int _y, float Fov)
{
    ...
    y = _y;
    FovH = Fov;
    FovV = 2.0f * atanf(tanf(Render::deg2rad(Fov) * 0.5f) * (float)y / (float)x);
...
void Camera::set_look_at(float4 _camera_center, float4 _focal_center, float4 _up, int mode)
{
    if(mode == Render::RIGHT)
    {
        ...
        up = std::move(_up);
        float4 Z = (focal_center - camera_center).normalized(); //朝向物体
