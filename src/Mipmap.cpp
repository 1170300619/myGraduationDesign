...
float4 Mipmap::sample_bilinear(const float2 &texture_uv, int level) const
{
    ...
    float4 color_x0 = Render::lerp(ratio_u, color_1, color_0);
    float4 color_x1 = Render::lerp(ratio_u, color_3, color_2);
    return Render::lerp(ratio_v, color_x0, color_x1);
}
...
float4 Mipmap::sample_trilinear(const float2 &texture_uv, float level, int low) const
{
    ...
    return Render::lerp(level - (float)low, low_color, high_color);
}
