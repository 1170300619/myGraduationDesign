...
public:
	...
	void backProjection(float4 point, float4 _camera_center, float4 _focal_center, float4 up, int X, int Y, int mode = Render::RIGHT);

	//void calcCorreponse(vector<float4> &camPos, vector<float4>_focal_center, vector<float3> &landmarks, vector<float4>_up, const string &resPath);
    /**
